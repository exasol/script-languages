...
#install Python3 packages via pip with legacy dependency resolver, as customers might already use this container with their own (pinned) dependencies which won't work without this option (the new resolver is more restrictive)
RUN /scripts/install_scripts/install_via_pip.pl --file /build_info/packages/flavor_customization/python3_pip_packages --python-binary python3.7 --with-versions --allow-no-version --use-deprecated-legacy-resolver
