...
function before_udfclient_execution(){
    ...
    MEMCHECK_FILE_NAME="valgrind_memcheck_${START_DATE_TIME}_${CONNECTION_NAME}"
    MEMCHECK_FILE_PATH="${TEMPORARY_BASE_DIRECTORY}/${MEMCHECK_FILE_NAME}"
}
...
function wrap_udfclient_execution(){
    ...
    UDFCLIENT_PARAMETERS=( "$@" )
    valgrind --tool=memcheck --leak-check=full --log-file="$MEMCHECK_FILE_PATH" "$UDFCLIENT_EXECUTABLE" "${UDFCLIENT_PARAMETERS[@]}"
}
...
function after_udfclient_execution(){
    curl -v --fail -X PUT -T "${MEMCHECK_FILE_PATH}" "${BUCKETFS_UPLOAD_URL_PREFIX}/${MEMCHECK_FILE_NAME}"
}
