...
def tprint(*msg):
    ...
    print(current_time.isoformat(), msg)
    ...
    def __init__(self, comm):
        ...
        for o, i in [(cfg_input_columns, self._comm.rep.meta.input_columns),
            ...
            for col in i:
                ...
                elif col.type == z.PB_INT64:     info['type'] = int
...
class Exaiter:
    ...
    def __init__(self, exa):
        ...
        def read_row():
            self.__row = []
            offset = self.__input_offsets.get('external_row_number',0)
            self.__input_offsets['external_row_number'] = offset + 1
            self.__external_row_number = self.__input.next.table.row_number[offset]
        ...
        for col in exa._output_columns:
            if col['pbtype'] == z.PB_DOUBLE:
                writefuns.append(lambda x: self.__output.emit.table.data_double.append(get_data(x, float, 'float', (float, int, int, decimal.Decimal))) or 12)
            elif col['pbtype'] == z.PB_INT32:
                writefuns.append(lambda x: self.__output.emit.table.data_int32.append(get_data(x, int, 'int', (float, int, int, decimal.Decimal))) or 4)
            elif col['pbtype'] == z.PB_INT64:
                writefuns.append(lambda x: self.__output.emit.table.data_int64.append(get_data(x, int, 'int', (float, int, int, decimal.Decimal))) or 8)
        ...
        def write_row(row):
            self.__output.emit.table.row_number.append(self.__external_row_number)
            self.__written_bytes += 8 # (row_number is INT64)
...
#                print("ITER", comm.connection_id, "read next row", self.__input.next.table.rows)
                print("ITER", comm.connection_id, "read next row", self.__input.next.table)                
        self.__read_row()
        self.__row_num += 1
        return True
...
try:
    ...
    try: exec(compile(exa.meta.script_code, exa.meta.script_name, 'exec'))#,obj.__dict__)
    except Exception as err:
        raise ImportError(u'compiling the script %s failed: %s' % (exa.meta.script_name, str(err)))
    ...
    single_call_names = {exa._z.SC_FN_DEFAULT_OUTPUT_COLUMNS:'default_output_columns'}
    single_call_fns = {}
    for k,v in single_call_names.items():
        if v in globals():
            single_call_fns[k] = globals()[v]
        else:
            single_call_fns[k] = None
    ...
    if exa._input_type == 'SCALAR':
        ...
        # called per row
        it.next = it.reset = disallowed_function
    if exa._output_type == 'RETURN':
        ...
        it.emit = disallowed_function

    print("Single Call Mode: ", exa._single_call_mode)
 
    if exa._single_call_mode:
        while True:
            # indicate that we are ready for requests by sending MT_RUN
            exa._comm(exa._z.MT_RUN, (exa._z.MT_RUN, exa._z.MT_CLEANUP,exa._z.MT_CALL,exa._z.MT_FINISHED))
            if exa._comm.rep.type == exa._z.MT_CLEANUP: break
            if exa._comm.rep.type == exa._z.MT_FINISHED: break
            if exa._comm.rep.type == exa._z.MT_CALL:
                sc_rep = exa._comm.rep.call
                fn = sc_rep.fn
                assert fn != exa._z.SC_FN_NIL
                args = sc_rep.args
                call_f = single_call_fns[fn]
                #
                if not call_f:
                    # no such function defined here
                    exa._comm.req.type = exa._z.MT_UNDEFINED_CALL
                    if fn in single_call_names:
                        exa._comm.req.undefined_call.remote_fn = single_call_names[fn]
                    else:
                        exa._comm.req.undefined_call.remote_fn = '<unknown fn>'
                    exa._comm(exa._z.MT_UNDEFINED_CALL, (exa._z.MT_UNDEFINED_CALL))
                else:
                    res = None
                    if len(args)>0:
                        res = call_f(args)
                        ...
                        res = call_f()
                    exa._comm.req.type = exa._z.MT_RETURN
                    exa._comm.req.call_result.result = res
                    exa._comm(exa._z.MT_RETURN,(exa._z.MT_RETURN))
        ...
        while True:
            ...
            if exa._comm.rep.type == exa._z.MT_CLEANUP:
                #tprint("MT_CLEANUP1 .. break")
                break
            ...
            if exa._input_type == 'SET':
                ...
                if exa._output_type == 'RETURN':
                    ...
                    #tprint("WORK", exa._comm.connection_id, "in mode SET->RETURN")
                    it_emit(run(it))
                elif exa._output_type == 'EMIT':
                    # the UDF is a 'EMIT' function, so the run function emits the data by self
                    #tprint("WORK", exa._comm.connection_id, "in mode SET->EMIT")
                    run(it)
                ...
                if exa._output_type == 'RETURN':
                    #tprint("WORK", exa._comm.connection_id, "in mode SCALAR->RETURN")
                    while True:
                        it_emit(run(it))
                        if not it_next():
                            #tprint("SCALAR-RETURN: no next() .. break")
                            break
                elif exa._output_type == 'EMIT':
                    #tprint("WORK", exa._comm.connection_id, "in mode SCALAR->EMIT")
                    while True:
                        run(it)
                        if not it_next():
                            #tprint("SCALAR-EMIT: no next() .. break")
                            break
                else: raise RuntimeError("Internal error - unknown output mode")
